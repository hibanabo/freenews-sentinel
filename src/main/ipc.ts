...
import { store, Keyword, Alert, Brief, Settings } from './store'
import { DEFAULT_PROMPT_PREFIX } from './constants'
import * as db from './db'
import { applyCachedSecrets, hasSecureSecretStorage, saveSecrets, stripSecrets, withSecrets } from './secrets'
...
import { showAlertNotification } from './window'
import type { TopicRunRecord } from './store'
...
function effectiveQuotaRemaining(quota: FreenewsQuotaSnapshot): number {
  return quota.effectiveRemaining ?? Math.max(0, Math.min(quota.dailyRemaining, quota.monthlyRemaining))
}

function isUnlimitedQuotaValue(value: number | null | undefined): boolean {
  return value === -1
}
...
function normalizePreviewItem(item: Record<string, unknown>): TopicQueryPreviewItem | null {
  ...
  if (typeof item.publishedAt === 'string' && item.publishedAt) {
...
async function fetchNewsDetail(id: number, apiKey: string, baseUrl: string): Promise<TopicNewsDetail> {
  ...
  return {
    ...
    publishedAt: json.data.publishedAt ? new Date(json.data.publishedAt).toISOString() : null,
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle('get-settings', async () => {
    ...
    const systemLang: 'zh' | 'en' = systemLocale.toLowerCase().startsWith('zh') ? 'zh' : 'en'
  ...
  ipcMain.handle('trigger-mock-alert', (_, level: MockAlertLevel = 'high') => {
    ...
    const alert: Alert = {
      ...
      articleSource: realArticleSource || 'Test data',
